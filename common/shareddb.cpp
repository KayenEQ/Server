...
void SharedDatabase::LoadSpells(void *data, int max_spells) {
	*(uint32*)data = max_spells;
    ...
    for (auto row = results.begin(); row != results.end(); ++row) {
		...
		sp[tempid].mana_ratio = atoi(row[152]); //C!Kayen
		...
		sp[tempid].GFX=atoi(row[182]); //C!Kayen
		...
		sp[tempid].no_wear_off_msg = atoi(row[199]) != 0;//C!Kayen
		...
		sp[tempid].CasterRestriction = atoi(row[220]);
