		...
		switch (effectid)
		{
			...
			case SE_DamageShield:
			{
				...
				if (base2)//C!Kayen - Just set it as base2 value in spdat.h
					new_bonus->DamageShieldType = static_cast<DmgShieldType>(base2);
				else{
			...
			{
				if (new_bonus->PetLifeShare[1] < effect_value){
					new_bonus->PetLifeShare[1] = effect_value; //Mitigation
					new_bonus->PetLifeShare[0] = buffslot;
					new_bonus->PetLifeShare[2] = base2; //Penalty
					new_bonus->PetLifeShare[3] = max; //Rune Amt
				}
				break;
			}
					
			case SE_BaseSpellPower:
				new_bonus->BaseSpellPower += effect_value; 
				break;

			case SE_BaseSpellPowerDmg:
				new_bonus->BaseSpellPowerDmg[base2] += effect_value; //Base2 is resist type
				break;

			case SE_BaseSpellPowerHeal:
				new_bonus->BaseSpellPowerHeal += effect_value; 
				break;

			case SE_BaseSpellPowerWizard:
				new_bonus->BaseSpellPowerWizard += effect_value; 
				break;

			case SE_MitigateAllDamage:
				new_bonus->MitigateAllDamage += effect_value; 
				break;

			case SE_MeleeLifeTapPetOwner:
				new_bonus->MeleeLifeTapPetOwner += effect_value; 
				break;
			
			case SE_MeleeManaTapPetOwner:			
				new_bonus->MeleeManaTapPetOwner += effect_value; 
				break;

			case SE_EffectField:			
				new_bonus->EffectField = effect_value; 
				break;

			case SE_HealAggroMod:			
				new_bonus->HealAggroMod += effect_value; 
				break;

			case SE_SpellAggroMod:			
				new_bonus->SpellAggroMod += effect_value; 
				break;

			case SE_AuraField:			
				new_bonus->AuraField = effect_value; 
				break;

			case SE_IncomingSpellDmgPct:{
				if (base2 == -1)
					new_bonus->IncomingSpellDmgPct[HIGHEST_RESIST] += effect_value;
				else
					new_bonus->IncomingSpellDmgPct[base2] += effect_value; //Base2 is resist type
				break;
			}

			case SE_FastManaRegen:{
				new_bonus->FastManaRegen[0] = effect_value;
				new_bonus->FastManaRegen[1] = base2;
				new_bonus->FastManaRegen[2] = max;
				new_bonus->FastManaRegen[3] = spell_id;
				break;
			}

			case SE_MeleeManaTap:
				new_bonus->MeleeManaTap += effect_value;
				break;

			case SE_MeleeEndurTap:
				new_bonus->MeleeEndurTap += effect_value;
				break;

			case SE_ScaleMitigationNumhits:
				new_bonus->ScaleMitigationNumhits[0] = effect_value;
				new_bonus->ScaleMitigationNumhits[1] = buffslot;
				new_bonus->ScaleMitigationNumhits[2] = base2;
				break;

			case SE_ScaleDamageNumhits:
				new_bonus->ScaleDamageNumhits[0] = effect_value;
				new_bonus->ScaleDamageNumhits[1] = buffslot;
				if (base2 == -1)
					new_bonus->ScaleDamageNumhits[2] = HIGHEST_SKILL + 1;
				else
					new_bonus->ScaleDamageNumhits[2] = base2;
				break;

			case SE_ScaleHitChanceNumhits:
				new_bonus->ScaleHitChanceNumhits[0] = effect_value;
				new_bonus->ScaleHitChanceNumhits[1] = buffslot;
				if (base2 == -1)
					new_bonus->ScaleHitChanceNumhits[2] = HIGHEST_SKILL + 1;
				else
					new_bonus->ScaleHitChanceNumhits[2] = base2;
				break;

			case SE_RangerGainNumhitsSP:
				new_bonus->RangerGainNumhitsSP[0] += effect_value;
				new_bonus->RangerGainNumhitsSP[1] = buffslot;
				new_bonus->RangerGainNumhitsSP[3] = spell_id;
				
				if (!new_bonus->RangerGainNumhitsSP[2] || base2 >= new_bonus->RangerGainNumhitsSP[2])
					new_bonus->RangerGainNumhitsSP[2] = base2;
			...
			case SE_ChargeEffect:
				...
				new_bonus->ChargeEffect[2] = base2;
				break;

			case SE_CriticalHitChanceFlank:
			{
				if(base2 == -1)
					new_bonus->CritHitChanceFlank[HIGHEST_SKILL+1] += effect_value;
				else
					new_bonus->CritHitChanceFlank[base2] += effect_value;
			...
			case SE_LifeShare:
			{
				if (new_bonus->LifeShare[1] < effect_value){
					new_bonus->LifeShare[1] = effect_value; //Mitigation
					new_bonus->LifeShare[0] = buffslot;
					new_bonus->LifeShare[2] = base2; //Penalty
					new_bonus->LifeShare[3] = max; //Rune Amt
				}
				break;
			}

			case SE_DistanceRemovalFromCaster:
				new_bonus->DistanceRemovalFromCaster = true;
				break;

			case SE_IncommingCriticalMelee:
				new_bonus->IncommingCriticalMelee += effect_value;
				break;

			case SE_AbsorbMeleeDamage:{

				new_bonus->AbsorbMeleeDamage[0] = effect_value;
				new_bonus->AbsorbMeleeDamage[1] = buffslot;
				new_bonus->AbsorbMeleeDamage[2] = base2;
				new_bonus->AbsorbMeleeDamage[3] = max;
			...
			case SE_SpinAttack:{
				new_bonus->SpinAttack[0] = effect_value;
				new_bonus->SpinAttack[1] = base2;
				break;
			}

			case SE_BottomHateList:
				new_bonus->BottomHateList = true;
				break;
